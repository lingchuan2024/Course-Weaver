...
- 可选调用 DeepSeek/Kimi 参与知识点抽取，识别父主题、学习阶段、前置依赖和易混淆关系，并将结构化草稿改写成更自然的讲义正文。
...
- `--use-llm` 会先让模型基于带页码和 block id 的解析结果抽取知识单元，并输出 `teaching_role`、`learning_stage`、`parent_topic`、`prerequisites`、`confusable_with` 等结构字段，再改写 `note_chunks` 正文；如果 AI 抽取失败，会回退到本地启发式抽取。
...
知识树读取 `project.note_plan`、`project.knowledge_units` 和 `project.relations`，按“Lecture 根节点 -> 章节主干 -> 知识点叶子 -> 关系边”展示。`note_plan` 会先利用 AI 抽取出的 `prerequisites` 建依赖图，再用带教学阶段优先级的拓扑排序生成学习路径；如果图中存在环，会保留稳定顺序兜底。用户滚动中间笔记时，右侧路径图会自动高亮当前节点，已经读过的节点会标记为完成。`next` 表示学习顺序，`foundation_for`、`contrasts_with`、`regularizes`、`example_of`、`supports` 等关系挂在对应章节节点下。溯源页用于查看选中 block 的原文、页码、进入笔记的位置和关联知识点。漏洞页优先展示 missing/uncertain，再展示 merged/ignored，作为人工复核清单。
